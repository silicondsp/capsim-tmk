...
button .addButton -text "Add Custom Block" -command { add_star $capsim }
button .delButton -text "Delete Block" -command { delete_star   $capsim .list   }
...
button .viewCCodeButton -text "View C  Code" -command { view_star_ccode $capsim}
...
button .genHTMLButton -text "View HTML" -command { view_star_html $capsim }
...
label  .labelCustom -text " Block Database Operations"
label  .labelStarName -text "Block Name"
...
proc delete_star  {capsim listname} {
  global .list
  set star [list_selected  $listname ]
  if { $star == "" } {return}
  
  puts "Deleting from database blockdatabase.dat ..."
  puts $star
  set theCommand {perl }
  append  theCommand $capsim
  append theCommand "/TOOLS/blockmaint.pl d "
  ...
  set theCommand {perl }
  append  theCommand $capsim
  append theCommand "/TOOLS/blockmaint.pl g "
  puts $theCommand
...
proc supplied_to_custom  { capsim } {
  ...
  puts "Adding to Database Operations List ..."
  puts $star
...
#
# This is a double check to see if blockmaint.pl was really able to add
#
set fileName result.txt
set found 0
if { [file readable $fileName ] } {
...
if { $found == 0}  {
   puts { Library Block already in database }    
...
proc view_star_ccode  { capsim } {
  ...
  puts "Viewing Block C Code ..."
  puts $star
 
# java -jar $(CAPSIM)/TOOLS/saxon.jar block.s $(CAPSIM)/TOOLS/blockgen.xsl>block.c
  ...
  set theBlockCCommand $capsim
  append theBlockCCommand "/TOOLS/blockgen.xsl > "
  append theBlockCCommand $starC
  
  set theBlockJavaGenCommand {java -jar }
  append theBlockJavaGenCommand $capsim
  
  append theBlockJavaGenCommand "/TOOLS/saxon.jar  "
  append theBlockJavaGenCommand  $star
  append theBlockJavaGenCommand  {  }
  append theBlockJavaGenCommand  $theBlockCCommand
  
  puts $theBlockJavaGenCommand
  eval exec $theBlockJavaGenCommand
...
# Procedure to view block HTML,

proc view_star_html  { capsim } {
  global .list
  set star [list_selected  .list ]
  if { $star == "" } {
      set star [.starNameEntry get ]
...
# java -jar $(CAPSIM)/TOOLS/saxon.jar block.s $(CAPSIM)/TOOLS/blockgen.xsl>block.c
  ...
  set theBlockCCommand $capsim
  append theBlockCCommand "/TOOLS/blockhtml.xsl > "
  append theBlockCCommand $starHTML
...
grid config .genHTMLButton -column 5 -row 9 -sticky w
