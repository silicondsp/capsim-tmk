...
# Modified Sasan Ardalan switched to XML for stars (parameters, buffers, main code, wrapup etc.) 
#
...
#
    ...
    if (/^<LICENSE>/ .. /^<\/LICENSE>/) {
	if ($Fld[1] ne '<LICENSE>' && $Fld[1] ne '</LICENSE>' && $#Fld != 0) {
	    $licensecode{$no_license++} = $_;
	}
    }


    if (/^<COMMENTS>/ .. /^<\/COMMENTS>/) {
	if ($Fld[1] ne '<COMMENTS>' && $Fld[1] ne '</COMMENTS>' && $#Fld != 0) {
	...
	if ($Fld[1] eq '<STATES>' || $Fld[1] eq '<STATES>' || $Fld[1] eq '</STATES>' ||
...
# DYNAMIC support
	...
	#Throw out the dynamic and end line and blank lines
	if ($Fld[1] eq '<DYNAMIC>' || $Fld[1] eq '<DYNAMIC>' || $Fld[1] eq '</DYNAMIC>' ||
	  ...
	  $#Fld == 0) {
	    ...
	    #Determine if dynamic variable is initialized
	    # by looking for a "=" in the line
	    if ((@line2 = split(/=/, $line1[1], 9999)) == 2) {
		$initialize = 1;
	    }
		...
		if ($line3[1] eq 'struct') {
		    ...
		    &Pick('>', 'sg.error') &&
			(print $fh "STARGAZE: error in dynamic line:\n" . $_);
		}
	    }
		...
		$dynamicv{$no_dynamic} = $line3[2] . ' =' . $line2[2];

		#To avoid macro recursion, generate a new name for the
		#dynamic variable in the structure by prefixing "__"
		#to the original name
		;
	    }
	    $new = '__' . $line3[2];
	    ...
	    #Generate code to go in state structure
	    $dynamicv{$no_dynamic++} = $line3[1] . ' ' . $new;

	    # detect dynamic variable arrays (ljf)
	    if ((@line4 = split(/\[/, $line3[2], 9999)) == 2) {
		$line3[2] = $line4[1];
		$new = '__' . $line3[2];
	    }
	    #Generate define statement
# FIX	    $define{$no_define++} = '#define ' . $line3[2] . "   \t(dynamic_P->"
# FIX
# FIX	      . $new . ')';
	}
    }
	...
	if ($Fld[1] ne '<DECLARATIONS>' && $Fld[1] ne '<DECLARATIONS>' &&

	  $Fld[1] ne '</DECLARATIONS>' && $#Fld != 0) {
	...
	if ($Fld[1] ne '<INIT_CODE>' && $Fld[1] ne '</INIT_CODE>' &&
	  $#Fld != 0) {
	    $initcode{$no_init++} = $_;
	}
    }

    if (/^<MAIN_CODE>/ .. /^<\/MAIN_CODE>/) {
	if ($Fld[1] ne '<MAIN_CODE>' && $Fld[1] ne '</MAIN_CODE>') {
	    $mcode{$no_main++} = $_;
	}
    }
    if (/^<WRAPUP_CODE>/ .. /^<\/WRAPUP_CODE>/) {
	if ($Fld[1] ne '<WRAPUP_CODE>' && $Fld[1] ne '</WRAPUP_CODE>' && $#Fld != 0) {
	    $wcode{$no_wrapup++} = $_;
	}
    }
...
&Pick('>', $out_file) &&
    (print $fh "#include \"../include/capsim.h\"");
&Pick('>', $out_file) &&
    (print $fh "#include \"../include/stars.h\"");
&Pick('>', $out_file) &&
    (print $fh '');
...
if ($no_dynamic > 0) {
    &Pick('>', $out_file) &&
	(print $fh 'typedef struct {');
    for ($i = 0; $i < $no_dynamic; ++$i) {
	&Pick('>', $out_file) &&
	    (print $fh "\t" . $dynamicv{$i} . ';');
    }
    &Pick('>', $out_file) &&
	(print $fh "\t} dynamic_t,*dynamic_Pt;");
}
...
#print license  
    &Pick('>', $out_file) &&
	(print $fh "\n\n");
for ($i = 0; $i < $no_license; ++$i) {
    &Pick('>', $out_file) &&
	(print $fh $licensecode{$i});
}
&Pick('>', $out_file) &&
	(print $fh "\n\n");
#print comments  
...
if ($no_dynamic > 0) {
    #Print dynamic variable allocation

    &Pick('>', $out_file) &&
	(print $fh "\tdynamic_Pt  dynamic_P; ");

    &Pick('>', $out_file) &&
	(print $fh "\tif(block_P->dynamic_P) dynamic_P = (dynamic_Pt)block_P->dynamicBlock_P->dynamic_P;");

    #Print state variable initialization
    for ($i = 0; $i < $no_dynamic; ++$i) {
	if ($dynamicv_init{$i} ne '') {
	    &Pick('>', $out_file) &&
		(print $fh "\t" . $dynamicv_init{$i} . ';');
	}
    }
}
...
if ($no_dynamic > 0) {
&Pick('>', $out_file) &&
    (print $fh '/********* DYNAMIC INITIALIZATION CODE ************/');
&Pick('>', $out_file) &&
    (print $fh 'case DYNAMIC_INIT:');
&Pick('>', $out_file) &&
    (print $fh "\nreturn(sizeof(dynamic_t));");
