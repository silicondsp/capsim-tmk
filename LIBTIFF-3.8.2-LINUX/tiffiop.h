/* $Header: /opt/XCAD/iip/TIFF/tiffiop.h,v 1.1.1.1 2003/12/28 01:52:30 root Exp $ */
...
typedef	int (*TIFFVoidMethod)(TIFF*);
typedef	int (*TIFFIOMethod)(TIFF*, u_char*, u_long, u_int);
typedef	int (*TIFFSeekMethod)(TIFF*, u_long);
#else
typedef	int (*TIFFVoidMethod)();
typedef	int (*TIFFIOMethod)();
typedef	long (*TIFFSeekMethod)();
#endif
...
#define	TIFF_POSTENCODE		0x200	/* need call to postencode routine */
	...
	long	tif_dataoff;		/* current offset for writing dir */
/* tiling support */
...
/* compression scheme hooks */
	TIFFVoidMethod	tif_predecode;	/* pre row/strip/tile decoding */
	TIFFVoidMethod	tif_preencode;	/* pre row/strip/tile encoding */
	TIFFVoidMethod	tif_postencode;	/* post row/strip/tile encoding */
	TIFFIOMethod	tif_decoderow;	/* scanline decoding routine */
	TIFFIOMethod	tif_encoderow;	/* scanline encoding routine */
	TIFFIOMethod	tif_decodestrip;/* strip decoding routine */
	TIFFIOMethod	tif_encodestrip;/* strip encoding routine */
	TIFFIOMethod	tif_decodetile;	/* tile decoding routine */
	TIFFIOMethod	tif_encodetile;	/* tile encoding routine */
...
extern	const TIFFFieldInfo *TIFFFindFieldInfo(u_short, TIFFDataType);
extern	const TIFFFieldInfo *TIFFFieldWithTag(u_short);
...
#endif
#else /* !USE_PROTOTYPES */
extern	void* _TIFFmalloc();
extern	void* _TIFFrealloc();
extern	void _TIFFfree();
extern	int _TIFFgetMode();
extern	const TIFFFieldInfo *TIFFFindFieldInfo();
extern	const TIFFFieldInfo *TIFFFieldWithTag();
