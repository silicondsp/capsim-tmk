...
static unsigned long ReadLong(fp)
FILE *fp;
{
...
float	*y_P;
float	*x_P;
long int	blockSize,offsetSound;
...
while(bytesRead <formChunk_P->ckSize && !feof(fp)  && !getOut) {
	...
	if(chunkID == formatCkID ) {
			...
			for(i=0; i<numberMarkers; i++) {
				...
				if(hdr_P)
				   IIPMarkerAdd(markerName,position,id,hdr_P);
			}
			if(hdr_P)
				IIPMarkerPrint(hdr_P);
