...
/**********************************************************************
			nor()
***********************************************************************
Function performs logical "nor" of all its input samples
...
<![CDATA[ 
	...
	if((ibufs = NO_INPUT_BUFFERS()) < 1) {
		fprintf(stderr,"nor: no input buffers\n");
		return(2);
	}
	if((obufs = NO_OUTPUT_BUFFERS()) < 1) {
		fprintf(stderr,"nor: no output buffers\n");
		return(3);
	}
...
<![CDATA[ 
	...
	/* read one sample from each input buffer and "nor" them */
