...
Function performs logical "or" of all its input samples
...
case USER_INIT: 
	...
	if((ibufs = NO_INPUT_BUFFERS()) < 1) {
		fprintf(stderr,"or: no input buffers\n");
		return(2);
	}
	if((obufs = NO_OUTPUT_BUFFERS()) < 1) {
		fprintf(stderr,"or: no output buffers\n");
		return(3);
	}
...
case MAIN_CODE: 
	...
	/* read one sample from each input buffer and "or" them */
