...
<![CDATA[ 
	...
	if((obufs = NO_OUTPUT_BUFFERS()) > ibufs) {
		fprintf(stdout,"imgwrtiff: more output than input buffers\n");
		return(2);
	}
	for(j=0; j<obufs; j++)
...
for(ii = MIN_AVAIL(); ii>0; ii--) {
	...
	pwidth=img.width;
	mat_PP=img.image_PP;
	fprintf(stderr,"imgwrtiff to produce %d x  %d TIFF image file\n",pwidth,pheight);
	if(IIP_WriteMatrixTIFFText(mat_PP,pwidth,pheight,theFileName,colorMapFile)) {
                fprintf(stderr,"imgwrtiff: can't write TIFF image\n");
                return(4);
        }

	if(obufs==1) {
                if(IT_OUT(0)) {
			KrnOverflow("imgwrtiff",0);
			return(99);
		}
                OUTIMAGE(0,0) = img;
        }
