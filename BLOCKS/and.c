...
Function performs logical "and" of all its input samples
...
case USER_INIT: 
	...
	if((ibufs = NO_INPUT_BUFFERS()) < 1) {
		fprintf(stderr,"and: no input buffers\n");
		return(2);
	}
	if((obufs = NO_OUTPUT_BUFFERS()) < 1) {
		fprintf(stderr,"and: no output buffers\n");
		return(3);
	}
...
case MAIN_CODE: 
	...
	/* read one sample from each input buffer and "and" them */
	for(samples = MIN_AVAIL(); samples >0; --samples) {
		...
		for(i=0; i<obufs; i++) {
			if(IT_OUT(i)) {
				fprintf(stderr,"and: buffer %d overflow\n",i);
				return(99);
			}
			OUTF(i,0) = sample_out;
		}
